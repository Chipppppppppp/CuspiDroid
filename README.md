...
- 複数タブとタブ・ブックマーク一覧の段階表示、未ロードのブックマークも含めてタブ一覧と同じ順番で表示し上下配置・長押し削除・未読数表示（設定でオフ可能）・右端での新規タブ追加とタブ一覧共通の未読数更新に対応するタブバー、最後のページ・タブ一覧・新規タブから選べる起動画面、ブックマーク、履歴、未読管理、閲覧位置の保存・復元、未読合計を表示するホーム画面ウィジェット
- 5chとカスタムBBSの板一覧・板・スレッドの段階表示、スレッドURLの長押しコピー、したらばを含む対応掲示板で新着投稿だけを追加する更新とスレッドを開いた際の新着読み込み、スワイプ更新、一覧の並べ替え
- URL入力、スレッド検索、ページ内検索、次スレ検索、必死チェッカー
- 書き込み、スレッド作成、掲示板認証、名前・メール履歴、スレッド・レス番号・名前・IDを確認できる新しい順の書き込み履歴
- 既読の後に未読をまとめる返信ツリー、未読境界への自動移動、返信・リンク・人気レスの抽出、スレッドとレス番号を確認できカテゴリ別に新しい順で閲覧できるお気に入りレス、AA表示、コピペ省略、NG管理
- 標準・AI判定・投稿や返信の注意語別に表示／ぼかし／非表示を選べる画像・GIF・動画のプレビューと拡大、ImgBBへのアップロード
- ピュアホワイト／ピュアダークを含むライト／ダーク対応のテーマ、作成・共有・バックアップできるカスタムテーマ、カテゴリ別に検索できる設定、検索・タイトル・タブバーそれぞれの上／下／非表示を選べる画面・ジェスチャー設定
- ネイティブ表示とWebViewの切り替え、プライベートブラウジング
- スレッド・メディアのキャッシュと各種履歴の個別管理
- Sync2ch同期、全設定と利用データのバックアップ作成・復元、ChMateバックアップの取り込み

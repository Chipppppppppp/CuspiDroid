...
- 複数タブとタブ・ブックマーク一覧の段階表示、未ロードのブックマークも含めてタブ一覧と同じ順番で表示し上下配置・長押し削除・未読数表示（設定でオフ可能）・右端での新規タブ追加とタブ一覧共通の未読数更新に対応するタブバー、最後のページ・タブ一覧・新規タブから選べる起動画面、ブックマーク、履歴、未読管理、閲覧位置の保存、未読合計を表示するホーム画面ウィジェット
- 5chとカスタムBBSの板一覧・板・スレッドの段階表示、スレッドURLの長押しコピー、新着投稿だけを追加する更新とスレッドを開いた際の新着読み込み、スワイプ更新、一覧の並べ替え

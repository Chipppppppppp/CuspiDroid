...
- 5ch・エッヂなどの板タイトルバーから、通常の書き込みと同じUIで本文を投稿しながらスレッドを作成し、作成したスレッドを新しいタブで自動的に開けるアプリ内書き込み。エッヂで認証が必要な場合はコードを表示・コピーしてWebViewを開き、認証後に元の書き込みを自動再送。名前・メール履歴、自分の書き込み履歴、認証が必要な掲示板向けのWebView連携

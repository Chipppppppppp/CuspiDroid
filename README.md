...
- 通常表示からWebViewへ切り替えても同じ画面内にアプリ本体の検索バーを維持し、検索候補・デザイン・ボタン配置・メニューをそのまま使用。タブ追加・タブ一覧・ブックマーク操作だけを除外し、通常時の「WebViewで開く」を「アプリに戻る」に置き換えてネイティブ表示へ復帰。5ch検索のHTTP 502はWebView内でも自動再試行

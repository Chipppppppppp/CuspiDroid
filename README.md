...
- タブ一覧の「すべて更新」はタイトル・レス数などの軽量情報だけを更新し、新着本文は取得せず高速に完了。新着が検出されたスレッドは、そのタブを開いた時に本文を遅延読み込み。閉じたブックマークフォルダーの合計未読数も、最新の既読状態を即座に反映
- URL 入力と検索を兼ねたアドレスバー、`find.5ch.io` またはカスタムURLによるスレッド検索。`find.5ch.io` が一時的にHTTP 502を返した場合は自動的に再試行し、復旧しない場合は全文検索経路から結果を取得
...
- 5chおよびカスタムBBSの板・スレッドをネイティブ表示し、引っ張って更新。板読み込みは他のバックグラウンドI/Oから分離し、エッヂなどのカスタムBBSは一覧を小分けに段階描画。スレッドから未キャッシュの板へ戻る場合も、即座に板のローディング画面へ移動。「5chリンクを新規タブで開く」が有効なら、スレッドの「板へ」も新しいタブで表示
...
- WebView表示でも従来と同じ検索・URL入力欄を使用し、タブ追加・タブ一覧・ブックマーク操作を表示せず、「アプリに戻る」ボタンでネイティブ表示へ復帰。5ch検索のHTTP 502はWebView内でも自動再試行

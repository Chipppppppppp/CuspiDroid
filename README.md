...
- ツリー表示、返信・リンク・閾値を変更できる人気レスの抽出、4行以上の本文を対象に空白行を除外し、行頭の空白・句読点・引用符・罫線などの連続と行内の連続空白をスコアリングして1行当たり0.8超なら適用するAA自動判定とAA表示、コピペ省略、自分の書き込みとそれへの返信を別色の左バーで示す強調表示、板単位にも対応したNGWord・NGName・NGID管理

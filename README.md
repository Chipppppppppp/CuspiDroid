...
- 長辺を基準に専用の正方形セル内へ全体が収まる画像・GIF・動画の共通プレビュー、スレ内メディアを指に追従して切り替える左右スワイプと全画面ズーム、スレ表示・メディア一覧での強いぼかしと拡大表示でさらに強くなるグロ画像判定のぼかし、各表示中央の「閲覧注意」ボタン、「グロ」「死ね」「閲覧注意」などの注意語を含む書き込みとその返信先の全メディアをぼかす既定オンの設定、ImgBBへのアップロードと同じ比率で縮小表示するアップロード履歴と、他のポップアップと統一した削除確認ダイアログ

...
- 次スレ検索では、同じ板のスレッド一覧から題名の類似度と連番の連続性を使って、候補を少数に絞って表示
- 書き込み長押し・メディア／リンク／人気レス抽出・次スレ候補では、枠線と背景暗転を統一したポップアップ表示
...
下記のBBSは、アプリの「設定」→「BBSリンク」→「BBSリンクを管理」→「対応BBSを一括追加」からまとめて登録できます。追加前には確認ダイアログが表示され、すでに登録済みの項目は上書きされません。BBSリンクは長押ししてドラッグすると並べ替えられます。

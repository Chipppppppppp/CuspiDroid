...
- 新しいセマンティック色を追加する場合は、インディゴ・ティール・グリーン・ブルー・ライトブルー・パープル・クリムゾン・ピンクそれぞれのライト／ダーク全組み込みパレット、`Theme.Palette` のJSON入出力、`Theme.COLOR_KEYS`、テーマ編集画面の表示名・GUIカラーピッカー・色プレビューを同時に更新し、既存のテーマファイルを読み込める互換性を維持すること。

...
- Shared image, GIF, and video previews scaled by their longest edge to fit fully inside dedicated square cells, with full-screen zoom, horizontal swiping between thread media, one on-device AI model that jointly detects inappropriate images including graphic and filthy content, a shared blur treatment and a warning before reveal, ImgBB uploads, upload-history media scaled with the same aspect-preserving behavior, and delete-confirmation dialogs consistent with the app's other popups
...
CuspiDroid is distributed under the GNU General Public License v3.0. See [LICENSE](LICENSE) for details. Attribution and licenses for bundled third-party AI models are listed in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

...
- Sixteen built-in themes: redesigned Default palettes plus Light and Dark variants for Teal, Green, Blue, Light Blue, Purple, Crimson, and Pink, with an automatic setting that switches between Default (Light) and Default (Dark) with the device theme. Every built-in theme follows the “Name (Light/Dark)” naming format, and the single selected theme is shared by normal and private browsing. The selector shows the current palette and every candidate with an eight-color preview, light/dark and theme type details, and a selection mark. Create custom themes from any built-in theme by editing 23 semantic colors—including screens, bars, cards, posts, fields, menus, text, borders, selections, link highlights, accents, the scroll sidebar and unread range, left bars for your posts and replies to them, tree connectors, and the low/high ends of metadata gradients—plus system-bar icon brightness. Loading indicators consistently use the selected theme's accent color. Each color supports direct `#RRGGBB` entry and a visual picker with hue presets, hue/saturation/brightness sliders, and a live preview; complete themes can then be imported or exported as JSON files. Also includes a top or bottom address bar and configurable buttons and swipe gestures

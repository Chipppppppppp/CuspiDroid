...
- Shared image, GIF, and video previews scaled by their longest edge to fit fully inside dedicated square cells, with finger-tracking horizontal swipes between thread media, full-screen zoom, strong graphic-image blurring in post previews and the media list, an even stronger blur in the expanded viewer, centered “Sensitive” buttons across all three views, a default-on option that blurs every media item both in posts containing warning terms and in posts targeted by those replies, ImgBB uploads, upload-history media scaled with the same aspect-preserving behavior, and delete-confirmation dialogs consistent with the app's other popups

...
- Combined URL and search address bar with thread search through `find.5ch.io` or a custom URL. Temporary HTTP 502 responses from `find.5ch.io` are retried automatically, with the full-text search path used as a fallback if the service does not recover
...
- WebView keeps the regular search and URL field while omitting new-tab, tab-overview, and bookmark controls, and provides a "Return to app" button for returning to the native view. HTTP 502 responses from 5ch search are also retried inside WebView

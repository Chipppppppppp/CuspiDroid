...
- Four built-in themes—Light, Dark, Private Browsing, and a selectable blue-based sample—plus an option to follow the device theme. Themes can be assigned independently to normal and private browsing. The improved selector shows the current palette and every candidate with a five-color preview, light/dark and theme type details, and a selection mark. Create custom themes from any built-in theme by editing 15 semantic colors—including screens, bars, cards, posts, fields, menus, text, borders, selections, link highlights, and accents—plus system-bar icon brightness, then import or export them as JSON files. Also includes a top or bottom address bar and configurable buttons and swipe gestures

...
- Shared image, GIF, and video previews scaled by their longest edge to fit fully inside dedicated square cells, with full-screen zoom, horizontal swiping between thread media, on-device AI blur detection for graphic and fecal images plus a warning before reveal, ImgBB uploads, upload-history media scaled with the same aspect-preserving behavior, and delete-confirmation dialogs consistent with the app's other popups

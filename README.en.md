...
- In-app posting with thread creation from the board title bar on 5ch, Edge, and other supported boards, using the regular posting UI for the initial message and automatically opening the created thread in a new tab. When Edge requires authentication, the app displays and copies the code, opens WebView, and automatically retries the original post after authentication. Also includes name/mail history, personal post history, and WebView integration for BBS authentication

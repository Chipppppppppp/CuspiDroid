...
- Switching from the native view to WebView keeps the main app's actual search bar in the same screen, including its suggestions, design, configured button placement, and menu structure. New-tab, tab-overview, and bookmark controls are omitted, while "Open in WebView" becomes "Return to app" for restoring the native view. HTTP 502 responses from 5ch search are also retried inside WebView

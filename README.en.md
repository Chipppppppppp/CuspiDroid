...
- Saved scroll/read positions, first-unread jumps, unread coloring, list metadata and sorting, with ranks transitioning from gray at No. 100 to maximum blue at No. 1 and post counts and speed becoming bluer as their values increase
- Tree view, reply/link/popular-post filters with an editable popularity threshold, automatic AA detection for bodies of at least four nonblank lines by scoring repeated leading whitespace, punctuation, quotation marks, box-drawing characters, logical operators, and set operators plus repeated in-line whitespace and requiring a per-line score above 0.75, AA rendering, repeated-copy omission, distinct colored left bars for your posts and replies to them, and board-scoped NGWord/NGName/NGID rules
...
- Ten built-in themes providing Light and Dark variants for each of Teal, Green, Blue, Purple, and Crimson, plus an automatic setting that switches between Teal (Light) and Teal (Dark) with the device theme. Every built-in theme follows the “Color (Light/Dark)” naming format, and themes can be assigned independently to normal and private browsing. The selector shows the current palette and every candidate with a five-color preview, light/dark and theme type details, and a selection mark. Create custom themes from any built-in theme by editing 15 semantic colors—including screens, bars, cards, posts, fields, menus, text, borders, selections, link highlights, and accents—plus system-bar icon brightness. Each color supports direct `#RRGGBB` entry and a visual picker with hue presets, hue/saturation/brightness sliders, and a live preview; complete themes can then be imported or exported as JSON files. Also includes a top or bottom address bar and configurable buttons and swipe gestures
- Switching from the native view to WebView keeps the main app's actual search bar in the same screen, including its suggestions, design, configured button placement, and menu structure. New-tab, tab-overview, and bookmark controls are omitted, while "Open in WebView" becomes "Return to app" for restoring the native view. Find-in-page also uses the same safe in-app search bar, with matches in threads, boards, and board lists highlighted using the same green background as the selected state while preserving the original text color
...
- Private browsing uses the dedicated dark-green theme by default. Its assignment is independent of normal tabs, consistently applying the selected built-in or custom theme to screens, search fields, posts, menus, icons, and borders while preserving private state in new tabs

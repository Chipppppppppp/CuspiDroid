...
- Ten built-in themes providing Light and Dark variants for each of Teal, Green, Blue, Purple, and Crimson, plus an automatic setting that switches between Teal (Light) and Teal (Dark) with the device theme. Every built-in theme follows the “Color (Light/Dark)” naming format, and themes can be assigned independently to normal and private browsing. The selector shows the current palette and every candidate with an eight-color preview, light/dark and theme type details, and a selection mark. Create custom themes from any built-in theme by editing 23 semantic colors—including screens, bars, cards, posts, fields, menus, text, borders, selections, link highlights, accents, the scroll sidebar and unread range, left bars for your posts and replies to them, tree connectors, and the low/high ends of metadata gradients—plus system-bar icon brightness. Each color supports direct `#RRGGBB` entry and a visual picker with hue presets, hue/saturation/brightness sliders, and a live preview; complete themes can then be imported or exported as JSON files. Also includes a top or bottom address bar and configurable buttons and swipe gestures
